<div class="relative w-full inline-block">
    <a href="{{ route('products.show', $product) }}">
        <div class="overflow-hidden w-full mb-2 relative rounded-lg">
            <img class="w-full object-cover rounded-xs group-hover:scale-110" src="{{ $product->thumbnailURL('thumb') }}"
                alt="{{ $product?->name }}" />
        </div>
    </a>
    <div>
        <a href="{{ route('products.show', $product) }}">
            <h2 class="text-base/6 text-gray-700 mb-2.5">
                {{ $product?->name }}</h2>
        </a>
        ...
        <form action="{{ route('products.addToCart') }}" method="POST" class="w-full">
            ...
            <button class="btn-primary w-full gap-2">
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none"
                    ...
                    <path d="M12.9479 9.89539H15.4898" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
